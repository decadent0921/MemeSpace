...
import com.yupi.memespace.api.aliyunai.model.CreateOutPaintingTaskRequest;
...
import com.yupi.memespace.mapper.PictureMapper;
import com.yupi.memespace.service.SpaceService;
import com.yupi.memespace.service.UserService;
import com.yupi.memespace.utils.ColorSimilarUtils;
...
public class PictureServiceImpl extends ServiceImpl<PictureMapper, Picture>
    ...
    public PictureVO uploadPicture(Object inputSource, PictureUploadRequest pictureUploadRequest, User loginUser) {
        ...
        // 如果是更新图片，需要校验图片是否存在
        if (pictureId != null) {
            Picture oldPicture = this.getById(pictureId);//获取旧图片
        ...
        // 根据 inputSource 类型区分上传方式，运用模板方法模式
        ...
        //todo 这里可以使用clearPictureFile()清理COS中更新完剩的旧图片
    ...
    public QueryWrapper<Picture> getQueryWrapper(PictureQueryRequest pictureQueryRequest) {
        ...
        if (StrUtil.isNotBlank(searchText)) {//判断 searchText 是否为空
            //对应SQL语句：(name LIKE '%xxx%' OR introduction LIKE '%xxx%')
            //用and将后面两个条件（）起来，代表or是name或者 introduction，而不是...or introduction。
        ...
        //还有通过标签查询
        /**由于 tags 在数据库中存储的是 JSON 格式的字符串，
         * 如果前端要传多个 tag（必须同时存在才查出），需要遍历 tags 数组，
         * 每个标签都使用 like 模糊查询，将这些条件组合在一起。*/
        if (CollUtil.isNotEmpty(tags)) {//tag集合不为空
            //增强for遍历集合
            for (String tag : tags) {
