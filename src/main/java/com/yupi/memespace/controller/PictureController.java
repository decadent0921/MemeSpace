...
import com.yupi.memespace.api.aliyunai.AliYunAiApi;
import com.yupi.memespace.api.aliyunai.model.CreateOutPaintingTaskResponse;
import com.yupi.memespace.api.aliyunai.model.GetOutPaintingTaskResponse;
import com.yupi.memespace.api.imageserch.ImageSearchApiFacade;
...
import com.yupi.memespace.model.enums.PictureReviewStatusEnum;
import com.yupi.memespace.model.vo.PictureTagCategory;
import com.yupi.memespace.model.vo.PictureVO;
import com.yupi.memespace.service.PictureService;
import com.yupi.memespace.service.SpaceService;
...
public class PictureController {
    ...
    //初始化Caffeine本地缓存的容器（直接在controller层构造比较方便）
    ...
    public BaseResponse<Page<Picture>> listPictureByPage(@RequestBody PictureQueryRequest pictureQueryRequest) {
        long current = pictureQueryRequest.getCurrent();//当初有默认值，如果前端没有传值，就会使用默认值1。
    ...
    public BaseResponse<Page<PictureVO>> listPictureVOByPageWithCache(@RequestBody PictureQueryRequest pictureQueryRequest,
        ...
        // 构建缓存 key
        String queryCondition = JSONUtil.toJsonStr(pictureQueryRequest);//使用hutool工具将对象转换为一个JSON字符串
        String hashKey = DigestUtils.md5DigestAsHex(queryCondition.getBytes());
        String cacheKey = "memespace:listPictureVOByPage:" + hashKey;
        ...
        // 更新本地缓存
        LOCAL_CACHE.put(cacheKey, cacheValue);//时间在Caffeine容器创建时就设置了。
        // 更新 Redis 缓存，设置过期时间为 5 分钟
        valueOps.set(cacheKey, cacheValue, 5, TimeUnit.MINUTES);//最后一个传入参数是时间单位。
    ...
    /**在项目前期规模不大，没必要将标签和分类单独用数据表来维护了，؜
     * 直接在PictureController中写一个接口؜，返回预设的固定数据。*/
